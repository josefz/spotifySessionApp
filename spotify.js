		...
		spotifyApi.getMe().then( function ( data ) {
			...
			spotifyApi.getUserPlaylists( userID ).then( function ( data ) {
				if ( data.body.items.length == 0 ) {
					...
					var tracks = [];
					var allPtracks = []
					Promise.all( allP ).then( function ( allData ) {
							var artistsList = [];
							...
							Promise.all( allPtracks ).then( function () {
...
function getArtistsData ( api, list ) {
	return new Promise( function ( resolve, reject ) {
		...
		Promise.all( allP.map( reflect ) ).then( function ( data ) {
				var success = data.filter( function ( x ) {
					return x.status === "resolved";
				} );
				var failed = data.filter( function ( x ) {
